...
An enterprise-grade, event-driven fitness tracking and AI coaching application architected with **Spring Boot Microservices**, **Apache Kafka**, **MongoDB**, **React**, and **Google Gemini AI**.
...
┌────────────────────────┐
   ...
   ▼                              ▼                              ▼
┌──────────────┐              ┌──────────────┐              ┌────────────────┐
│ User Service │              │Activity Svc  │              │ Eureka Discovery│
...
* **Microservices Ecosystem**: Loosely coupled architecture registered with Netflix Eureka for dynamic discovery and routing.
* **Centralized Configuration**: Spring Cloud Config Server managing configuration profiles across environments.
* **Event-Driven AI Pipeline**: Non-blocking activity event ingestion via **Apache Kafka**; the activity service publishes workout metrics instantly without blocking the client.
* **AI-Powered Fitness Coaching**: Integrates **Google Gemini API** to generate real-time workout analysis, pacing assessments, dynamic heart rate estimates, target improvements, and recovery safety guidelines.
...
Start the services sequentially to ensure registration and config bindings initialize properly:

1. **Infrastructure**:
   * Start **Zookeeper** & **Apache Kafka** (`localhost:9092`)
...
Add your Gemini API credentials to configserver/src/main/resources/config/ai-service.yml or your local profile:
